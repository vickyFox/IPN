...
class CCMNet(nn.Module):
    def __init__(self,
        ...
        self.num_layers = 1
        self.lstm1 = nn.GRU(in_features * 5, hidden_features * 5, num_layers=self.num_layers, batch_first=True)
        self.lstm2 = nn.GRU(in_features * 5, hidden_features * 5, num_layers=self.num_layers, batch_first=True)
    ...
    def forward(self, in_feat, num_supports):
        # in_feat: num_tasks(batch_size) x num_samples(26) x in_features(128)
        batch_size = in_feat.size(0)
        in_features = in_feat.size(2)
        ...
        for i in range(5):
            ...
            for j in range(5):
                ...
                output1, h_n1 = self.lstm1(support_data[:,(5*i)+j :(5*i)+j+1,:].repeat(1,1,5), m_h_1)
                output2, h_n2 = self.lstm2(support_data[:,(5*i)+ 4 - j :(5*i)+ 5 - j,:].repeat(1,1,5), m_h_2)
